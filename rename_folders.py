    ...
    for subfolder in SUBFOLDERS:
        ...
        for action in actions:
            ...
            for folder in folders:
                ...
                os.mkdir(os.path.join(NEXT_DATA_FOLDER, subfolder, action, str(folder_nb+ratio)))
                # os.rename(os.path.join(PREVIOUS_DATA_FOLDER, subfolder, action, folder), os.path.join(PREVIOUS_DATA_FOLDER, subfolder, action, str(folder_nb+ratio)))
                files = os.listdir(os.path.join(PREVIOUS_DATA_FOLDER, subfolder, action, folder))
                for file in files:
                    json_previous_file = os.path.join(PREVIOUS_DATA_FOLDER, subfolder, action, folder, file)
                    json_next_file = os.path.join(NEXT_DATA_FOLDER, subfolder, action, str(folder_nb+ratio), file)
