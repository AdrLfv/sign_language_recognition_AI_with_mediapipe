...
class Data_vis:
    ...
    def rebuild_frame(self, results):
        window=[]
        for i, _ in enumerate(results[0:33*4]):
            if(i%4 !=0):
                window.append(results[i])
                window.append(results[i+1])
                window.append(results[i+3])
                window.append(results[i+4])

        for i, _ in enumerate(results[33*4, 33*4+468*3]):
            window.append(results[33*4+i])
            window.append(results[33*4+i+1])
            window.append(results[33*4+i+2])
        # left hand
        for i, _ in enumerate(results[33*4+468*3: 33*4+468*3+21*3]):
            if(i%3 !=0):
                window.append(results[33*4+468*3+i])
                window.append(results[33*4+468*3+i+1])
                window.append(results[33*4+468*3+i+2])

        # right hand
        for i, _ in enumerate(results[33*4+468*3+21*3:]):
            if(i%3 !=0):
                window.append(results[33*4+468*3+21*3+i])
                window.append(results[33*4+468*3+21*3+i+1])
                window.append(results[33*4+468*3+21*3+i+2])


        return window

    def launch_vis(self,res):
        #label_map = {label: num for num, label in enumerate(self.actions)}

        data = {}       
        #frame = list(self.rebuild_frame(res) )
        frame = list(res )
